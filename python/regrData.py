...
count = 0
for invS, demS, sdCF, eI1, eI2, cT1, cT2 in itertools.product([-1,0,1],[-1,0,1],[-1,0,1],[-1,0,1],[-1,0,1], [-1,0,1],[-1,0,1]):
    ...
    try:
        ...
        # Write log file
        with open("dat/logNoCoop.txt", "a") as file:
            file.write(str(count)+": ")
            file.write( "  ".join( [str(vv) for vv in  (invS, demS, sdCF, eI1, eI2, cT1, cT2) ]   ))
            file.write ("\n")
        solution.append( [ans] )
    except :
        print ("\n Infeasibility for:" , count,": ", (invS, demS, sdCF, eI1, eI2, cT1, cT2),"")
        bad.append(count)
    # if count == 2:
        # break
    count = count+1

with open("dat/badNoCoop.txt","a") as file:
    towrite = ",".join ([str(bb) for bb in bad])
    file.write(towrite)

with open("dat/solutionsNoCoop.txt","w") as file:
